...
postData(urlBack+'sunburst2', req_sun_inic_2).then(data => {
    data.name = "ODS";
...
function dibujar_sunburst_2(data) {
    ...
    grupito = d3.select('#grupo_sunburst_2');
    //grupito.transition().duration(1000).attr("opacity", 0);

    var sunburst_col = d3.select("#sunburst_col_2");
    var sunburst_row = d3.select("#sect1");
    ...
    var prev_g = svg.selectAll("#grupo_sunburst_2");
    ...
    const g = svg.append("g")
        .attr("transform", `translate(${width_sunburst / 2},${width_sunburst / 2})`).attr("id", "grupo_sunburst_2");
        ...
        path.attr("fill-opacity", d => {
            if (arcVisible(d.current)) {
                if (d.data.name == sdg_bur_id && sdg_bur_id != meta_seleccionada_primera_2) {
                    ...
                    d3.selectAll('#imagen_3meta_sun_2').attr('src', d => {
                    ...
                    barritas_meta.selectAll("#nombre_meta_en_barra_2")
                    ...
                    barritas_meta.select('#porcentaje_meta_en_barra_2')
                        .transition().duration(300)
                        .attr("opacity", 0);
                    ...
                    d3
                        .select('#desc_meta_sun_int_primera_2')
                        .transition().duration(300)
                        .style("opacity", 0);

                    d3.select('#desc_meta_sun_int_primera_2')
                        .transition().delay(300)
    ...
    function mouseover_sunburst() {
        ...
        if (act_sdg != sdg_bur_id) {
            ...
            d3.select('#desc_meta_sun_2').text("" + descripciones_metas["meta_" + max_meta.data.name.split("meta_")[1]] + ".")
            d3.select('#imagen_meta_sun_2').attr('src', "assets/Metas%20ODS/ODS%20" + max_meta.data.name.split("_")[1] + "/" + max_meta.data.name.split("meta_")[1].replace("_", ".") + ".png");
            d3.select('#nombre_meta_2').text(`${max_meta.data.name}`.toUpperCase().replace("_", " ").replace("_", "."));
            per = max_meta.data.value;
            perc = (100 / root_2_2.value) * per;
    ...
    function clicked(p) {
        ...
        if (p.parent != undefined && p.parent.data.name == "ODS" && nivel_profundidad_2 == 1) {
            ...
            d3.selectAll('#imagen_3meta_sun_2').data(p.children.slice(0, 1)).join().attr('src', d => {
            ...
            const sunburst_barrita_col = d3.select("#prueba_barrita_2");
